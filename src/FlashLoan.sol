...
contract Flashloan {
 ...
 function loopingSupply() public {
  ...
  // swap USC ke WETH = 3300 USDC swap ke WETH untuk mendapatkan 1 WETH
  // membayar flashloan 1 WETH

  IERC20(weth).approve(lendingPool, 2e18);
  IAavePool(lendingPool).supply(weth, 2e18, address(this), 0);

  IAavePool(lendingPool).borrow(usdc, 3300, 2, 0, address(this));

  IERC20(usdc).approve(router, 3300);
